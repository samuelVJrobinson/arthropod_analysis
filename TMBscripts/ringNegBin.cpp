...
// Helper function for calculating negative exponential decay curve
// template<class Type>
// double negExp(Type d, Type eta, Type rho, Type lambda){ //eta*exp(-(rho^2)*(d^lambda))
  // return eta*exp(-pow(rho,2)*pow(d,lambda));
// } 
...
{	
	...
	for(int i=0; i<ringDist.size(); i++){ //For each ring distance	
	  ...
	  // ringCoef(i) = negExp(ringDist(i),eta,exp(logRho),lambda); //Calculate coefficient
	}
