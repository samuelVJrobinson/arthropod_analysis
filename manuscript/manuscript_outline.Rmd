...
## Data collection
We used a set of `r sum(NtrapsPer$n)` pitfall traps installed at roadsides (minimum of 5 m away from the road edge, `r with(NtrapsPer, sum(n[trapLoc=='ditch')` traps) and in-field locations (`r with(NtrapsPer, sum(n[trapLoc!='ditch')`) across southern Alberta, Canada in 2017 (Figure \@ref(fig:siteMap)).
...
In-field traps were placed in canola crops (`r with(NtrapsPer, sum(n[trapLoc=='canola')`), wetlands (`r with(NtrapsPer, sum(n[trapLoc=='wetland')`), grassy field corners (`r with(NtrapsPer, sum(n[trapLoc=='pivot')`), and remnant prairie grassland (`r with(NtrapsPer, sum(n[trapLoc=='native')`).
...
_Pardosa_ use an active wandering predation strategy [@young1990], and may use road margins and wooded areas as habitat [@buddle2000; @drapela2011]. 
...
Finally, we calculated the R$^2$ for each group of terms in each model [@nakagawa2013; @nakagawa2017].
...
In particular, grass/wetland acted as a sink for _P. melanarius_, forest/shrubs acted as a sink for _P. distincta_, and road margins acted as a sink for _P. moesta_.
