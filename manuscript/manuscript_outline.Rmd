...
* We used three species of predatory arthropods that were present at very high abundances in the traps
  ...
  + Finally, _Phallangium opilio_ is a widely-distributed harvestment from the order Opilio
  + 
...
* _Phalangium opilio_ activity density was strongly influenced by trap location, with pivot corners and wetlands having the highest activity density (p = `r with(opilLinTerms, unboldRMD(p[loc=='Pivot']))`, `r with(opilLinTerms, unboldRMD(p[loc=='Wetland']))`)  
  ...
  + The temporal and spatial components, as in the other two species, were very strong (both p = `r with(opilSmTerms,unboldRMD(p[grepl('E,N',Smoother)]))`, and Easting 
...
```{r varComp, echo=FALSE, fig.cap='\\label{fig:varComp} Variance components of all models'}
ggplot(varComp,aes(x=term,y=std.dev))+geom_col()+theme_classic()+facet_wrap(~spp,ncol=1,scales='free_y')+
