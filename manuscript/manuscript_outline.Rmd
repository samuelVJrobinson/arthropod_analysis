...
* Functional regression plots of landscape composition may seem cryptic, but are fairly straightforward to interpret:
  + Positive responses to nearby landscape cover classes indicate that the cover class acts as a source (i.e. organisms spill over *from* it)
  + Neutral responses indicate that the cover class does not act as a significant source or sink
  + However, negative responses to nearby landscape cover classes indicate one of two things
  + 1) It indicates that the cover class acts as a sink (i.e. organisms spill over *into* it)
  + 2) It indicates that the cover class acts as a source, but is unoccupied (i.e. no organisms available to spill over)
  + 
...
*  _P. melanarius_ had a very strong random temporal and spatial component (both p<0.001), indicating that phenology and local geographic factors were strong drivers of ground beetle activity density (Figure \@ref(fig:PteMelSpatiotemporal))
...
* _Pardosa distincta_ activity density was also strongly influenced by trap location and landscape composition (Tables \@ref(tab:ParDisLinear), \@ref(tab:ParDisSmooth)), similar to _P. melanarius_.
  ...
  + Grassland acted as a weak source, at least earlier on in the season (p = `r with(pdisSmTerms,p[Smoother=='s(endDayMat):Grassland'])`), and urban cover acted as a weak sink at intermediate distances (~600m)
...
* _P. moesta_ was also heavily influenced by trap location and landscape composition (Tables \@ref(tab:ParMoeLinear), \@ref(tab:ParMoeSmooth))
  + Similar to the other Lycosid spider _P. distincta_, _P. moesta_ had much lower activity density in canola than any other trap location (Figure \@ref(fig:ParMoeLandscape)a)
  ...
  + This indicates that both _P. distincta_ and _P. moesta_ migrate out of canola into adjacent habitat in order to overwinter or oviposit
  ...
  + Urban land cover was a sink, but only at certain times of the year
...
* _Phalangium opilio_ activity density was strongly influenced by trap location, with pivot corners and wetlands having the highest activity density (p = `r with(opilLinTerms, p[loc=='Pivot'])`, `r with(opilLinTerms, p[loc=='Wetland'])`)  
  ...
  + This indicates that _P. opilio_ activity density is shaped mostly by large-scale geographic patterns, rather than landscape composition
...
* The predatory ground beetle _Pterostichus melanarius_ responded to landscape composition

* The Lycosid spiders _Pardosa distincta_ and _Pardosa moesta_ responded to landscape composition
