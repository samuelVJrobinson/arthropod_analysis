---
title: Landscape composition and beneficial insects - time is important
...
date: "Spring 2020"
abstract: Ground-dwelling arthropods are important generalist predators in prairie agro-ecosystems. However, we don't know what types of landscape features they live in. Some may act as sources of arthropods during some parts of the year, and sinks during others. We examined four widely-distributed species of arthropods, using functional regression to examine their habitat preferences. We found that trees/shrubs, and roadside grasses acted as sources of arthropods. Other features did not function as significant sources or sinks. Farmers should consider retaining these landscape features in order to preserve populations of helpful generalist predators in their crops.
...
Landscape structure drives beneficial arthropod abundance, and is key for understanding how to manage benefical insects
...
Contrast between landscape and local SNL is important [@boetzl2020; @vanSchalkwyk2020]; small patches can have a large effect if overall landscape lacks SNL
Management goal: be able to design landscapes that benefit both farmers and insect diversity/abundance
  
Abundance is also influenced by spatio-temporal processes, such as phenology and migration
...
1) Untilled semi-natural land (SNL) provides oviposition and feeding areas for beneficial arthropods, meaning that nearby SNL should act as a source of arthropods.
2) Crops in agricultural land attract pest insects during the growing season, and these insects are preyed upon by beneficial arthropods. 
However, pest phenology/crop senescence at the end of the season causes a decrease in the number of pests. 
This means that crop cover may act as a sink for beneficial insects early in the season as they migrate into the crop, and a source later on as they migrate out of the crop.
...
Cereal was removed from the model, as it was negatively correlated with canola cover at distances less than 200m and was positively correlated at distances over 500m, reflecting the most common crop rotations (canola -> wheat -> barley) and the landscape structure present across our study region.
This resulted in a much-reduced set of landscape terms, none of which were strongly concurved with each other (Figure \@ref(fig:concurvity)) 
...
Grassland, cereal, and canola were the three most-abundant landcover classes surrounding our traps, accounting for `r 27.44+25.76+22.65`% of land cover.
Several landscape ``fingerprints" were evident in the landscape annuli, with cereal cover increasing with distance away, along with a corresponding decrease in canola cover (leading to the strong concurvity).
...
Canola crops had a marginally higher activity density of _P. melanarius_ than other trap locations (Figure \@ref(fig:PteMelLandscape)a), possibly indicating that canola may allow faster movement of ground beetles.
In landscape composition, nearby grassland/wetland cover had a neutral effect early in the summer, but a negative effect later in the summer (Figure \@ref(fig:PteMelLandscape)b), indicating that _P. melanarius_ may move into grasslands later in the year.
Nearby canola had a positive effect in the early summer, but conversely, far-away canola (>800 m) had a negative effect, indicating that areas with widespread amounts of canola coverage had lower _P. melanarius_ activity density than those with simply local canola cover.
Two other crop types had unexpected effects on _P. melanarius_ activity density: pulses and flax.
Pulses acted as a late-season source, possibly indicating that _P. melanarius_ migrate out of the crop after (or during) harvest.
Flax had both a local and landscape-level negative effect, suggesting that the crop is not suitable for _P. melanarius_ foraging.
...
Activity density of _Pardosa_ was strongly influenced by trap location and landscape composition (Tables \@ref(tab:ParDisLinear), \@ref(tab:ParDisSmooth)).
...
Pulses had a similar effect on _P. distincta_ activity density, where landscape-level abundance of pulses increased _P. distincta_ density, but local abundance had a neutral effect.
...
_P. opilio_ was affected by grassland/wetlands (p=`r with(opilSmTerms,p[Smoother=='ti(Distance,Day):GrassWetland'])`), trees/shrubs (p`r with(opilSmTerms,p[Smoother=='s(Distance):TreeShrub'])`), and flax (p <`r with(opilSmTerms,p[Smoother=='s(Day):Flax'])`).
...
Flax had a negative influence, but this effect was only strong during the middle of the season (Figure \@ref(fig:OpilioLandscape)d).
...
+-----------------+----------------------------+----------------------------+-----------------------------+-----------------------+
| Forest & Shrubs |                            | - Overall sink (late)      | - _Overall source (early)_  | - Local sink (early)  |
|                 |                            |                            | - _Overall sink (late)_     |                       |
+-----------------+----------------------------+----------------------------+-----------------------------+-----------------------+
| Pulses          | - Overall source (late)    |                            | - Landscape source          |                       |
+-----------------+----------------------------+----------------------------+-----------------------------+-----------------------+
...
The predatory ground beetle _Pterostichus melanarius_ responded to landscape composition, primarily to different crop types (Table \@ref(tab:sumTab),\@ref(tab:PteMelSmooth)).
Of the non-crop land categories, grassland and wetland cover acted as a late-season sink, suggesting that _P. melanarius_ may migrate into it at the end of the summer.
...
The roles that carabids play in pulse crops is not well-studied, but carabids are predators of pea-leaf weevils (_Sitona lineatus_) on other Fabaceae [@hamon1990].
_P. melanarius_ is also known to prey upon other carabids that are predators of _S. lineatus_, such as _Bembidion quadrimaculatum_ [@vankosky2011].
Either way this suggests that pulses may represent a potential food resource for _P. melanarius_, at least earlier in the season.
...
Canola acted as an early-season source for both species, indicating that _Pardosa_ may use canola as foraging grounds.
Trees and shrubs acted as a late season sink in both species, suggesting that they migrate to off-field areas with woody shrubs and trees for overwintering [but _P. distincta_ are known to be active during the winter; @aitchison1984].
@drapela2011 also found that _Pardosa agrestis_ had higher activity density in areas with woody trees and shrubs.
@moring1994 showed that _P. distincta_ were more active in grassy substrates.
While _Pardosa_ are found in areas close to disturbed roadside strips [@drapela2008; @drapela2011; @kowal2011], _P. distincta_ appears to be less sensitive to disturbances than _P. moesta_ [@collins1996; @wade2010].
We found that roads clearly acted as an early source and a late sink of _P. moesta_.
Grassland and wetland, as well as pulses, also acted as a local sink, but a landscape-level source for _P. moesta_.
  
_Phalangium opilio_ activity density did not respond to landscape composition, aside from pasture (p=`r with(opilSmTerms,p[Smoother=='ti(distMat,endDayMat):Pasture'])`) and a weaker response to trees/shrubland (p=`r with(opilSmTerms,p[Smoother=='ti(distMat,endDayMat):TreeShrub'])`).
...
```{r landscapeComp, echo=FALSE, fig.cap='Landscape composition in annuli surrounding each trap location. Sites are represented by individual lines. Overall percent cover for each cover class is listed in each sub-figure heading.'}  
coverList %>% group_by(cover) %>% mutate(p=sum(prop)) %>% ungroup() %>% mutate(p=100*round(p/sum(unique(p)),4)) %>% 
  ...
  facet_wrap(~cover)+theme_classic()+labs(y='Percent cover',x='Distance (m)')
