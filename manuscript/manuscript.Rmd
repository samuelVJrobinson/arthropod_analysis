...
#Load ML model
load('../data/PteMelMod_ML.Rdata')
load('../data/ParDisMod_ML.Rdata')
load('../data/ParMoeMod_ML.Rdata')
load('../data/OpilioMod_ML.Rdata')

#Compare annular and radial models 
compareMods <- list(PteMelMod,ParDisMod,ParMoeMod,OpilioMod) %>% 
  sapply(.,function(x){
    with(x,c(mod2AIC=AIC(mod2),mod2df=attr(logLik(mod2),'df'),
      mod3AIC=AIC(mod3),mod3df=attr(logLik(mod3),'df'),
      mod5AIC=AIC(mod5),mod5df=attr(logLik(mod5),'df'),bestDist=mod5info$radiusDist[which.min(mod5info$AIC)]))
  }) %>% t() %>% data.frame() %>% 
  mutate(spp=c('Pterostichus melanarius','Pardosa distincta','Pardosa moesta','Phalangium opilio')) %>% 
...
pmelSmTerms <- pmelRes$s.table %>% as.data.frame() %>% 
  ...
  mutate(Smoother=gsub('Urban','Roads',Smoother)) %>% 
...
However, beneficial arthropods living in non-crop areas can increase crop yields by providing ecosystem services, such as pollination or pest suppression [@losey2006; @klein2007; @garibaldi2013].
...
Arthropods can disperse from managed crops back into adjacent SNL at the end of the season [@desender1988; @tscharntke2005b], but this is seldom considered.
Second, the scale at which the landscape is relevant to an organism depends on their traits and life history [@ahrenfeldt2015].
...
2. Crops provide food (pest insects) for predatory arthropods during the growing season. This should result in agricultural land becoming a destination for predators in the early part of the season as they disperse into the crop, and a source at the end of the season as they disperse out of the crop.
3. Crops may act as a temporary feeding site for predators. Therefore, crops may act as a nearby destination for arthropods, but will also be negatively associated at more-distant scales.
...
## Data collection
We used a set of `r sum(NtrapsPer$n)` pitfall traps across southern Alberta, Canada in 2017 (Figure \@ref(fig:siteMap)).
...
The data were collected as an offshoot of an earlier study that examined pollination services in crops [@vickruck2019], and here we use pitfall trap data to examine the predatory arthropod community with a focus on canola crops (\emph{Brassica napus} L.).
Canola is one of the most commonly-grown crops in southern Alberta, and represented 15\% of the total land cover within the spatial extent of our sampling region during 2017 (Figure \@ref(fig:siteMap)).
We did not collect information on soil fertilization, pesticide application, mowing, or tillage (approximately 82% of planting in Vulcan County was done with zero-till in 2016; [@statscan_tillage2016]). 
Diverse crop rotations may also increase predatory arthropod abundance [@busch_Msc; @bertrand2016], but this was beyond the scope of the study.

Pitfall traps were installed in road margins (minimum of 5 m away from the road edge, `r with(NtrapsPer, sum(n[trapLoc=='ditch']))` traps) and in-field in canola crops (`r with(NtrapsPer, sum(n[trapLoc!='ditch']))`).
Traps in canola were installed in 27 separate fields at 25, 75, and 200 m along a transect heading away from the nearest non-crop feature (wetland, grassy field edges, or remnant prairie), while the trap at 0 m was installed in the non-crop feature itself (see Figure \@ref(fig:siteMap) inset maps).
The remaining 5 sites had only a single trap.
In total, in-field traps were placed in canola crops (`r with(NtrapsPer, sum(n[trapLoc=='canola']))`), wetlands (`r with(NtrapsPer, sum(n[trapLoc=='wetland']))`), grassy field edges (`r with(NtrapsPer, sum(n[trapLoc=='pivot']))`), and remnant prairie grasslands (`r with(NtrapsPer, sum(n[trapLoc=='native']))`).
"Wetlands" were permanent (non-ephemeral) flooded areas, "field edges" were the grassy corners of central-pivot irrigated fields, and "remnant prairie" were uncultivated grasslands composed of native grasses and herbs. 
We used 582 mL Solo® cups buried up to the rim and partially filled with propylene glycol, with 2 cm wire mesh mounted over the rim to prevent small vertebrates from falling into the traps.
...
We used counts of organisms in our pitfall traps as proxies of pest control service.
However, increased counts of organisms in pitfall traps may represent higher activity levels (same number of organisms but more mobile) or higher abundances in the vicinity of the trap.
...
(ref:siteMapCap) Classified land cover and location of pitfall traps during 2017. Roadside traps were installed in road margins, while in-field traps were installed directly in canola fields, wetlands, or grassy field edges. Inset maps show study location within Alberta provincial boundaries, as well as examples of trap placement at two in-field sites.

```{r siteMap, echo=FALSE, fig.cap='(ref:siteMapCap)', out.width='100%'}  
  include_graphics('../figures/maps/PF2017Map.png',dpi=NA)
```
  
We used four highly-abundant species of predatory arthropods found in the pitfall traps, as common species are more important for ecosystem service provision than rare species [@winfree2015; @kleijn2015; but see @winfree2018].
...
To characterize landscape composition surrounding the traps, we used 30 m resolution classified landscape data [@AAFC_data].
Cropland classifications from 2017 were very accurate for cultivated fields (~90%), but were less accurate for non-crop areas (~70%).
...
At each site, we extracted the proportion of each cover class within 30 m annuli (rings), with the inner radius of each annulus ranging from 30 to 1470 m in increments of 30 m (total of 49 annuli, 1.5 km maximum).
...
Functional regression [@ramsay2004; @yen2014] was used to incorporate landscape information at different distances, allowing assessment of both nearby and far-away landscape composition [@galpern2020].
...
This allows for the possibility that the scale of landscape relevant to a given organism may change over the course of its life [@addicott1987; @lima1996; @gardiner2010].
It also allows for the possibility that small amounts of certain land cover types may be beneficial, but are detrimental at large scales, indicating that the cover type may not constitute a completely usable habitat.
...
Trapping location was included as a fixed effect with 5 levels (canola, field edge, grassland, road margin, and wetlands) to account for the effect of local cover independent of the effect of surrounding landscape composition (described below). 
Day of year was included as a 1-dimensional smooth, and easting and northing (km east and north of the trap extent centroid) were included as 2-dimensional smooths in order to account for underlying spatial and temporal autocorrelation in the data.

For each landscape cover class, we used three scalar-on-function terms.
First, we used the proportion cover in the annuli surrounding each trap location (spatial effect of cover class - _i.e._ does the effect of cover differ with distance?).
Second, we used the average proportion cover in all annuli across days of the year (temporal effect of cover class - _i.e._ does the effect of cover differ with time?). 
Finally, we used a tensor-product interaction of landscape cover and day of year (spatio-temporal interaction of cover class - _i.e._ does the effect of cover differ with distance and time?).
...
To remove unimportant terms from the model, we used thin-plate regression splines with extra shrinkage, a continuous analog to stepwise model selection where weaker terms are completely removed from the model rather than reduced to a line [@marra2011].
Deviance residuals from each model were visually inspected for normality and equal variance [@hilbe2011; @wood2017].
Finally, we calculated the proportion of explained variance (R$^2$) for each set of terms [@nakagawa2013; @nakagawa2017].

To compare scalar-on-function (annular) regression to the more traditional (radial) regression approach, we also fit radial models of arthropod counts using the same cover classes.
We fit count models using a smooth effect of proportion cover of radii of increasing distances around each trap location (30 - 1500 m, as above), and chose the model with the lowest AIC as the "best" model. 
Radial models reveal nothing about changes in the effect of land cover classes with distance or time, but serves to highlight differences in the predictive capability of the two methods.
As in the annular regression model, trapping location, log-days of trap placement, day of year, and easting and northing were also included in the radial models.
...
For highly concurved classes, we examined the spatial nature of the concurvity using scatterplots of proportion cover within each annulus to determine whether the cover classes should be merged (if classes were consistently related) or removed (if classes were inversely related to one another).
Forests and shrubland were combined into a single "woodland" category, and grassland and wetland were combined into a single "grassland" category, as they were strongly positively related to each other at all distances of annuli, indicating similar feature classes.
Cereal was removed from the model, as it was highly concurved with canola; in particular, it was negatively associated with canola cover at annulus distances of less than 200m and was positively associated at distances over 500m, reflecting the most common crop rotations (canola $\rightarrow$ wheat $\rightarrow$ barley) and the commonly-used 800 m block structure of farmland in our study region (quarter-sections).
...
Functional linear regression is a special case of linear regression, where the independent variable is predicted by a functional dependent variable, meaning that the slope is a function (vector) rather a fixed (scalar) value [@ramsay2004; @yen2014].
...
As functional regression takes any continuous predictor of _y_, it is also possible to use proportion cover as a predictor of measurements taken across time, allowing the effect of a (fixed) proportion cover on activity-density to vary over the course of the season (Figure \@ref(fig:frExample)d).
...
As mentioned above, abundance and activity are confounded within activity-density, meaning that our method cannot explicitly say whether changes in counts are due to changes in abundance (_i.e._ migration or reproduction) or changes in activity (_i.e._ foraging).
However, we consider it more likely that changes in activity-density related to landscape components are caused by changes in abundance, not activity, as there is no mechanism by which distant parts of landscape could alter activity. 
<!-- Changes in activity-density due to seasonal changes in foraging will be accounted for by the overall temporal smoother  -->

In the Results section, we refer to "near" and "far" landscape effects, where near indicates the effects of landscape composition at scales of <500 m, and far indicates scales of >500 m.
Similarly, we refer to "early" and "late"-season effects, where landscape composition affected activity-density in early June or late August, respectively.
...
Grassland cover was the most important land cover type in explaining activity-density ($R^2$: 0.104, Table \@ref(tab:rSquared)), and near grassland had a negative effect late in the summer (Figure \@ref(fig:PteMelLandscape)b), suggesting that _P. melanarius_ may move into grasslands later in the year.
Near canola had a positive effect in the early season, but landscape-level canola had a negative effect, indicating that areas with widespread canola coverage had lower _P. melanarius_ activity-density (Figure \@ref(fig:PteMelLandscape)c).
Pulses acted as a late-season source, indicating that _P. melanarius_ may disperse out of pulses after (or during) harvest (Figure \@ref(fig:PteMelLandscape)d).
Finally, _P. melanarius_ activity-density had a strong temporal and spatial component (both p `r with(pmelSmTerms,p[Smoother=='s(E,N)'])`), indicating that phenology and large-scale geographic factors were important drivers of ground beetle activity-density (Figure \@ref(fig:PteMelSpatiotemporal)).
...
_P. distincta_ activity-density was negatively affected by far pasture (Figure \@ref(fig:ParDisLandscape)b), suggesting that large amounts of pasture are unsuitable habitat for _P. distincta_.
There was also a negative effect of woodlands late in the summer (Figure \@ref(fig:ParDisLandscape)c) that did not depend on spatial scale, indicating that _P. distincta_ may disperse into woodlands at this time.
However, trap location and the temporal smoother (Figure \@ref(fig:ParDisSpatiotemporal)a) explained roughly 10 times more variance than landscape composition ($R^2$: `r round(sum(filter(r2Nakagawa2,model=='ParDisMod',terms=='trapLoc'|grepl('day',terms))$r2comp),3)` versus `r round(sum(filter(r2Nakagawa2,model=='ParDisMod',terms!='trapLoc',terms!='sigmaD',!grepl('s\\(',terms))$r2comp),3)`), meaning that while _P. distincta_ may be abundant in certain cover types, its overall habitat preferences are fairly general.
...
Grasslands had a near negative effect, but a positive far effect, indicating that these cover types act as a local destination, the general amount of grassland in the area had a positive effect (Figure \@ref(fig:ParMoeLandscape)b).
Near canola cover had a positive early effect on activity-density, but a negative effect later in the season (Figure \@ref(fig:ParMoeLandscape)c).
Near road margins also had a similar effect, acting as an early source and a late destination (Figure \@ref(fig:ParMoeLandscape)d).
...
(ref:ParMoeCap) Landscape influence on _Pardosa moesta_ activity-density. a) shows the effect of trap location, and b-e) are functional regression plots of the proportion grassland, canola, road margins, and pulses at distances away from each trap. Lines and dots represent means, and bars and shaded regions represent 95 % confidence regions (1.96 x SE). Coloured regions represent early- and late-season effects (blue = June 2, red = August 20).
...
_P. opilio_ activity-density was somewhat influenced by trap location, with field edges and wetlands having the highest activity-density (Figure \@ref(fig:OpilioLandscape)a), but this effect was dwarfed by the spatial and temporal smoothers (Table \@ref(tab:rSquared)), meaning that _P. opilio_ activity-density is weakly influenced by landscape composition.
Nearby grassland and woodland both had negative effects on _Phalangium opilio_ activity-density Figures \@ref(fig:OpilioLandscape)b,c), but the effect of grassland was negative only early in the season (only weakly for woodland: p=0.07), suggesting that _P. opilio_ may preferentially disperse to these cover classes.
The temporal smoother was overwhelmingly the most important predictor of _P. opilio_ activity-density ($R^2$: `r round(sum(filter(r2Nakagawa2,model=='OpilioMod',terms=='s(day)')$r2comp),3)`), showing that activity-density increased during the spring, and showed very little decline during the rest of the season (Figure \@ref(fig:OpilioSpatiotemporal)a).
Similar to _P. moesta_, _P. opilio_ also showed a distinct East-West spatial smoother (Figure \@ref(fig:OpilioSpatiotemporal)b), indicating that their activity-density is also influenced by larger-scale abiotic factors.

(ref:OpilioCap) Landscape influence on _Phalangium opilio_ activity-density. a) shows the effect of trap location, and b-c) are functional regression plots of the proportion grassland and woodland at distances away from each trap. Lines and dots represent means, and bars and shaded regions represent 95 % confidence regions (1.96 x SE). Coloured regions represent early- and late-season effects (blue = June 2, red = August 20).
...
r2Nakagawa2 %>% select(model,terms,r2comp) %>% mutate(terms=as.character(terms),r2comp=r2comp*1) %>%
  ...
  kable(.,format='latex',caption='R$^2_{GLMM}$ for model components, representing the proportion of total variance explained by each term. Trap Location refers to the individual cover type that the trap was installed in, Spatial Location and Day of Year refer to the overall spatial and temporal smoothers, Grassland through Road Margins refers to the functional smoothers over different surrounding land cover types, and Residual represents unexplained variation. R$^2$ was calculated using methods from Nakagawa et al. 2013 and Nakagawa et al. 2017.',
...
mytable <- data.frame(
  Species = c(' \\emph{Pterostichus}\n\\emph{melanarius}',' \\emph{Pardosa}\n\\emph{distincta}',
                              ' \\emph{Pardosa}\n\\emph{moesta}',' \\emph{Phalangium}\n\\emph{opilio}'),
    Summary = as.character(c('$\\cdot$ Likely oves to canola during early season \n$\\cdot$ May disperse to grassland during late season',
                '$\\cdot$ Negative effect of pasture \n$\\cdot$ May move locally to field edges',
                '$\\cdot$ Likely moves to canola during early season \n$\\cdot$ May disperse to road margins and grassland during late season \n$\\cdot$May disperse to grasslands across the season.',
                '$\\cdot$ Likely moves to grassland early in the season')),
...
mytable %>% mutate_all(linebreak) %>% 
  kable(.,format='latex',caption='Synthesis of model results. Pest control potential is based on likelihood of early-season activity-density in canola crops.',escape=FALSE,
      col.names=c('Species','Summary','Pest control potential'),align='lll') %>% 
  column_spec(2,width='15em')

```

## Comparison of annular and radial models

Functional regression provided a more parsimonious (lower AIC) activity-density model for _P. melanarius_, radial regression performed better for _P. moesta_ and _P. opilio_, and _P. distincta_ was modeled approximately equally by both model types (Table \@ref(tab:compareModels)).
The best radial model of _P. distincta_ used a landscape radius of 1410 m, adding weight to the interpretation of the functional regression model where far-away landscapes have an influence on _P. distincta_ activity-density, while the best landscape radii identified for _P. melanarius_ and _P. moesta_ were much smaller (`r compareMods$bestDist[grepl('melanarius',rownames(compareMods))]` and `r compareMods$bestDist[grepl('moesta',rownames(compareMods))]` m, respectively). 
Interestingly, the radial models of _P. moesta_ and _P. opilio_ had lower AIC scores than the functional regression model ($\Delta$AIC: `r with(compareMods,round(mod5AIC[3]-mod3AIC[3],3))`, `r with(compareMods,round(mod5AIC[4]-mod3AIC[4],3))`), suggesting that a simpler model may be more appropriate for some species than others (if the goal is purely prediction with the least number of variables).
However, none of the radial models accounted for the temporal effects of specific land cover classes, so it is difficult to compare the models directly.
All models had a lower AIC score than the null model, indicating that landscape composition is at least somewhat important for predicting activity-density. 


```{r compareModels, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}

compareMods %>% #select(-bestDist) %>% 
  rownames_to_column(var='Species') %>% 
  # rowwise() %>% mutate(whichMin=which.min(c(mod2AIC,mod3AIC,mod5AIC))) %>%
  kable(.,format='latex',caption='Comparison of models. The null model includes trap location and a spatial and a temporal smoother, but contains no landscape terms. The annular model includes the terms in the null model plus scalar-on-function landscape terms. The radial model includes the terms the null model plus radial landscape smoothers. Radius indicates the radius of landscape composition used in the best-fitting radial model.',escape=FALSE,
      col.names=c('Species','AIC','Edf','AIC','Edf','AIC','Edf','Radius (m)'),digits=1) %>% 
  add_header_above(c(' '=1,'Null Model'=2,'Annular Model'=2,'Radial Model'=3)) %>% 
...
This suggests that these arthropods disperse to these habitats at the end of the summer to prepare for oviposition or overwintering.
...
The influence of landscape composition tended to be limited to a radius of about 500m, indicating that nearby landscapes are associated with beneficial arthropod abundance.
...
Finally, the functional regression technique provided a more parsimonious model for _P. distincta_, highlighting its potential value for future work in landscape ecology.
<!-- In particular, functional regression provided inference about how landscape cover influences the spatio-temporal distribution of ecosystem services within agroecosystems. -->
...
<!-- 2. ... should result in agricultural land becoming a destination for predators in the early part of the season as they disperse into the crop, and a source at the end of the season as they disperse out of the crop. -->
<!-- 3. ... crops may act as a nearby destination for arthropods, but will also be negatively associated at far-away scales. -->
...
Early in the season, canola crops had a near positive effect, but a landscape-level negative effect, which may indicate that they provide only temporary feeding habitat.
...
Grassland and pulses also had a negative nearby effect but a positive faraway effect; this suggests that they may constitute suitable habitat for _P. moesta_ and attract them away from other cover types.
...
_P. opilio_ activity-density was higher in grassy field edges and wetlands, but grasslands and wetlands had a negative nearby effect on _P. opilio_ until late in the season.
...
_P. opilio_ is also found in large numbers at the edges of forests, and may disperse from forests into farmlands [@vandepoel2015Msc].
...
However, there was also a strong East-West pattern in the spatial smoother for both _P. opilio_ and _P. moesta_, meaning that larger-scale geographic patterns (rainfall or temperature) are more important influences on their activity-density.
...
We have shown how a relatively novel statistical technique can be used to consider multiple spatial scales of landscape composition, providing richer inference about the processes acting on beneficial arthropods.
Our results show indirect empirical evidence of arthropod movement between cover types at different times of year, which is rarely considered beyond single-field studies.
Since we tracked activity-density and not individual movements, future studies should directly examine arthropod movement and life-history within matrices of cover types, with the goal of integrating landscape ecology and behavioural processes into a single model [@lima1996].
...
While nearby habitat appears valuable, even far-away habitat can be valuable, as it can increase the number of beneficial arthropods at larger spatial scales (e.g. Figures \@ref(fig:PteMelLandscape), \@ref(fig:ParMoeLandscape)).
Second, our work suggests that _P. melanarius_ and _P. moesta_ should be investigated for their role as predators in canola crops (Table \@ref(tab:resultsTab)).
